...
void pose_fushion(float T)
{
  double Z[9],Z_flag[5]={0};
	...
	Z[1]=0;
	//yaw
	Z[2]=vmc_all.att[YAWr]*DEG_TO_RAD;
	//spd
	Z[3]=0;
	Z[4]=0;
	//w
	Z[5]=vmc_all.att_rate[Zr]*DEG_TO_RAD;
	//encoder
	Z[6]=vmc_all.param.encoder_spd[R];
	Z[7]=vmc_all.param.encoder_spd[L];
	//L
	Z[8]=vmc_all.W/2;
	...
	Z_flag[4]=1;//encoder
	if(nav.init[Xr])
		ekf_pos(X_kf_pose, P_kf_pose,  Z, Z_flag, Q_pose, R_pose,  T);
